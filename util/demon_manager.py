...
"""
demon_manager.py class courtesy of ericpretzel on github under wid-bot project
...
def generate_demon_report(server: int, data: dict):
    ...
    """
    server_folder = os.path.join(setter.DEMONS_FOLDER, str(server), )
...
def generate_sentences(server: int, user: int, num_sentences: int) -> str:
    ...
    """
    filepath = os.path.join(setter.DEMONS_FOLDER, str(server), f'{user}.json')
